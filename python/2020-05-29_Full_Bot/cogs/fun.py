...
class Fun(commands.Cog):
    ...
    async def mastermind(self, ctx):
        ...
        embed.add_field(
            name="Mastermind",
            value=
            "Enter a 5-character combination, using the characters `A`, `B`, `C` and `D`.",
            inline=False)
        ...
        while True:
            ...
            if correct == len(code):
                ...
                embed.add_field(
                    name="You win!",
                    value="The correct code was `{}`. <@{}> has won!".format(
                ...
                if attempts >= 1:
                    ...
                    embed.add_field(
                        ...
                        .format(correct, ("s" if correct != 1 else ""),
                                ("were" if correct != 1 else "was"), attempts,
                                ("es" if attempts > 1 else "")),
                        inline=False)
                    await ctx.send(embed=embed)
        embed = botembed()
        embed.add_field(
            name="You lose!",
            value="You lose! The correct code was `{}`".format(code),
            inline=False)
        await ctx.send(embed=embed)
