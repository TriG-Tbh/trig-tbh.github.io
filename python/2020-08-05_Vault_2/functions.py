...
def decrypt(path=None, delete=True, silent=False):
    ...
    double_encrypted = zlib.decompress(base64.b64decode(double_encrypted))
    single_encrypted = fernet.decrypt(double_encrypted)
    text = single_encrypted.decode('utf-8').strip()

    #print(single_encrypted[-5:])
    text = double_encrypted.decode("utf-8")
    name = os.path.split(path)[-1]
